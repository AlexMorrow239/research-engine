...
const studentInfoSchema = z.object({
  ...
  gpa: z.coerce
    ...
    .refine((val) => val >= 0.1 && val <= 4.0, "GPA must be between 0 and 4.0"),
  graduationDate: z
    .string()
...
const availabilitySchema = z.object({
  ...
  thursdayAvailability: z.string().min(1, "Thursday availability is required"),
  fridayAvailability: z.string().min(1, "Friday availability is required"),
  saturdayAvailability: z.string().min(1, "Saturday availability is required"),
  ...
  desiredProjectLength: z.nativeEnum(ProjectLength, {
    errorMap: () => ({ message: "Please select your desired project length" }),
  }),
});
